...
# sensitivity_pol.R
#
# Full pipeline for the "pol / full-length fragments only" sensitivity analysis.
#
...
# outputs suffixed "_pol" so nothing overwrites the main results.
#
# Outputs: RMSDcountries_pol.csv, migration_RMSD_pol.csv,
#          migration_adj_RMSD_pol.csv, migration_adj_RMSD_absdiff_pol.csv,
#          indices_pol.csv, table2_pol.docx, table4_standardised_pol.docx
# =====================================================================
...
# =====================================================================
# SENSITIVITY ANALYSIS: pol / full-length fragments only
#
# Records are kept only where every genotyping-fragment column is either
# missing or begins with "pol" or "full length".
# Filter reproduced verbatim from the original sensitivity_analysis.Rmd.
# =====================================================================
message("Before pol filter: ", nrow(HIV_samples), " rows")

HIV_samples <- HIV_samples %>%
  filter(startsWith(`Genotyping fragment`, "pol")   | startsWith(`Genotyping fragment`, "full length")   | is.na(`Genotyping fragment`)) %>%
  filter(startsWith(`Genotyping fragment.1`, "pol") | startsWith(`Genotyping fragment.1`, "full length") | is.na(`Genotyping fragment.1`)) %>%
  filter(startsWith(`Genotyping fragment.2`, "pol") | startsWith(`Genotyping fragment.2`, "full length") | is.na(`Genotyping fragment.2`)) %>%
  filter(startsWith(`Genotyping fragment.3`, "pol") | startsWith(`Genotyping fragment.3`, "full length") | is.na(`Genotyping fragment.3`)) %>%
  filter(startsWith(`Genotyping fragment.4`, "pol") | startsWith(`Genotyping fragment.4`, "full length") | is.na(`Genotyping fragment.4`)) %>%
  filter(startsWith(`Genotyping fragment.5`, "pol") | startsWith(`Genotyping fragment.5`, "full length") | is.na(`Genotyping fragment.5`)) %>%
  filter(startsWith(`Genotyping fragment.6`, "pol") | startsWith(`Genotyping fragment.6`, "full length") | is.na(`Genotyping fragment.6`)) %>%
  filter(startsWith(`Genotyping fragment.7`, "pol") | startsWith(`Genotyping fragment.7`, "full length") | is.na(`Genotyping fragment.7`)) %>%
  filter(startsWith(`Genotyping fragment.8`, "pol") | startsWith(`Genotyping fragment.8`, "full length") | is.na(`Genotyping fragment.8`)) %>%
  filter(startsWith(`Genotyping fragment.9`, "pol") | startsWith(`Genotyping fragment.9`, "full length") | is.na(`Genotyping fragment.9`))

message("After pol filter:  ", nrow(HIV_samples), " rows")
...
write.csv(countries_sampled, "counts_country_subtype_distirbutions_pol.csv")
...
write.csv(proportions, "proportions_pol.csv")
...
print(results)
write.csv(results, "RMSDcountries_pol.csv")
...
write.csv(indices, "indices_pol.csv")
...
RMSD <- read.csv("RMSDcountries_pol.csv") %>%
  mutate(
...
write.csv(migration_RMSD, "migration_RMSD_pol.csv", row.names = FALSE)
...
write.csv(net_flow_adj, "net_flow_adj_pol.csv", row.names = FALSE)
...
write.csv(migration_adj_RMSD, "migration_adj_RMSD_pol.csv", row.names = FALSE)
...
write.csv(migration_adj_RMSD_absdiff, "migration_adj_RMSD_absdiff_pol.csv", row.names = FALSE)
...
migration_RMSD<- read.csv("migration_RMSD_pol.csv")
#migration_RMSD_old<- read.csv("migration_RMSD_old.csv")

# population adjusted migration variable 
migration_adj_RMSD<- read.csv("migration_adj_RMSD_pol.csv")

#pop adjusted migration plus absolute difference in prevalence variable
migration_adj_RMSD_absdiff<- read.csv("migration_adj_RMSD_absdiff_pol.csv")
...
modelsummary(
  ...
  fmt       = fmt_statistic(estimate  = "%.2e",
                            ...
                            p.value   = "%.3f"),
  stars     = c('*' = .1, '**' = .05, '***' = .01),
  output    = "table4_standardised_pol.docx"
)
